...
class Exporter:
    ...
    def handleError(self, error):
        print ("Oops! Please report the following error to the developer:")
...
class Setup:
    ...
    def checkAvailableSize (self , problem):
        ...
        if requiredSize > free:
            print("You do not have enough space on your disk to continue with export." )
            print ("You need at least " + str("{:5.2f}".format(requiredSize * 0.000001)) + " MB of free space.")
            print ("You currently have " + str("{:5.2f}".format(free * 0.000001)) + "MB available on your chosen disk." )
            return True
        else:
            return problem
    ...
    def testScenarios(self, problem):
        if hasattr(self, "type"):
            if (forMunicipalBudget and not self.type == 2 ):
                print("Wrong data type for the Municipal Budget tool.")
                return True
            elif (forReachability and not self.type == 1 ):
                print("Wrong data type for the Reachability tool.")
                return True
        else:
           return problem    
    ...
    def testFieldNameInput (self, problem):
        try: 
            ...
            print("Oops! Field is not in layer.")
            return True
    ...
    def testFiles (self, problem):
        ...
        try:
            ...
            print("We could not open " + filePath + ". Please close the file")
            return True          
    ...
    def testMunicipalBudget (self, problem):
        if iface.activeLayer().geometryType() == 2 or not forMunicipalBudget:
            ...
            print("Oops! Municipal Budget can only be done from polygons.")
            return True 
...
class FileWriter:
    ...
    def __init__(self,raster, setup):
        if forReachability:
            ...
            print ("Data import done. Have a good day!")
...
class CheckLayer:
    ...
    def municipalBudgetLayer (self, setup):
        ...
        if setup.type == 2:
            print ("You are importing data for the Municipal Budget tool.") 
            ...
            FileWriter( rasterNoData,setup)
            if setup.isCanceledAndUpdateProgress(100.0): return None
            print ("Data import for the Municipal Budget tool is done. Have a good day!")
        else: 
    ...
    def rasterToUnits(self,setup, raster):
        ...
        if not QgsRasterLayer(reprojectedRaster,"Reprojected Raster").isValid():
            print("There is not enough storage on your disk for this size and resolution.")
    ...
    def aggregateAndSum(self, setup,rasterPath):
        ...
        if sizePercent >= 1: # this means that the new raster will be 1% (or more) smaller
            print("\nWARNING: the output raster will be smaller (" + str(outWidth) + " by " + str(abs(outHeight)) + "). That is " + "{:.1f}".format(sizePercent).replace(".0", "") + "% smaller\n")
